...
public class PinotInstanceAssignmentRestletResourceTest extends ControllerTest {
  ...
  public void testInstanceAssignment()
    ...
    try {
      sendGetRequest(_controllerRequestURLBuilder.forInstancePartitionsForTable(RAW_TABLE_NAME));
      fail();
    ...
    Map<InstancePartitionsType, InstancePartitions> instancePartitionsMap = JsonUtils
        .stringToObject(sendGetRequest(_controllerRequestURLBuilder.forInstancePartitionsForTable(RAW_TABLE_NAME)),
    ...
    instancePartitionsMap = JsonUtils
        .stringToObject(sendGetRequest(_controllerRequestURLBuilder.forInstancePartitionsForTable(RAW_TABLE_NAME)),
            new TypeReference<Map<InstancePartitionsType, InstancePartitions>>() {
            });
    ...
    instancePartitionsMap = JsonUtils.stringToObject(sendGetRequest(_controllerRequestURLBuilder
            .forInstancePartitionsForTable(TableNameBuilder.OFFLINE.tableNameWithType(RAW_TABLE_NAME))),
        new TypeReference<Map<InstancePartitionsType, InstancePartitions>>() {
        });
    assertEquals(instancePartitionsMap.size(), 1);
    assertTrue(instancePartitionsMap.containsKey(InstancePartitionsType.OFFLINE));
    instancePartitionsMap = JsonUtils.stringToObject(sendGetRequest(_controllerRequestURLBuilder
            .forInstancePartitionsForTable(TableNameBuilder.REALTIME.tableNameWithType(RAW_TABLE_NAME))),
        new TypeReference<Map<InstancePartitionsType, InstancePartitions>>() {
        });
    assertEquals(instancePartitionsMap.size(), 2);
    ...
    // Test fetching instance partitions by instance partitions name
    for (InstancePartitionsType instancePartitionsType : InstancePartitionsType.values()) {
      InstancePartitions instancePartitions = JsonUtils.stringToObject(sendGetRequest(_controllerRequestURLBuilder
              .forInstancePartitions(instancePartitionsType.getInstancePartitionsName(RAW_TABLE_NAME))),
          InstancePartitions.class);
      assertEquals(instancePartitions.getName(), instancePartitionsType.getInstancePartitionsName(RAW_TABLE_NAME));
    }

    // Remove the instance partitions for both OFFLINE and REALTIME table
    sendDeleteRequest(_controllerRequestURLBuilder.forInstancePartitionsForTable(RAW_TABLE_NAME));
    try {
      sendGetRequest(_controllerRequestURLBuilder.forInstancePartitionsForTable(RAW_TABLE_NAME));
      fail();
    } catch (FileNotFoundException e) {
    ...
    instancePartitionsMap = JsonUtils.stringToObject(
        sendPostRequest(_controllerRequestURLBuilder.forInstanceAssign(RAW_TABLE_NAME, null, true), null),
        new TypeReference<Map<InstancePartitionsType, InstancePartitions>>() {
        });
    assertEquals(instancePartitionsMap.size(), 3);
    ...
    instancePartitionsMap = JsonUtils.stringToObject(sendPostRequest(
    ...
    completedInstancePartitions = JsonUtils.stringToObject(sendPostRequest(_controllerRequestURLBuilder
        .forInstanceReplace(InstancePartitionsType.COMPLETED.getInstancePartitionsName(RAW_TABLE_NAME),
            offlineInstanceId, realtimeInstanceId), null), new TypeReference<InstancePartitions>() {
    });
    assertEquals(completedInstancePartitions.getInstances(0, 0), Collections.singletonList(realtimeInstanceId));

    // Replace the instance again using REALTIME table name (old instance does not exist)
    try {
      sendPostRequest(_controllerRequestURLBuilder.forInstanceReplace(InstancePartitionsType.COMPLETED
              .getInstancePartitionsName(TableNameBuilder.REALTIME.tableNameWithType(RAW_TABLE_NAME)), offlineInstanceId,
    ...
    consumingInstancePartitions = JsonUtils.stringToObject(
        sendPutRequest(_controllerRequestURLBuilder.forInstancePartitionsPost(),
            JsonUtils.objectToString(consumingInstancePartitions)), InstancePartitions.class);
    assertEquals(consumingInstancePartitions.getInstances(0, 0), Collections.singletonList(realtimeInstanceId));
